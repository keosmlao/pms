  ...
  const updated = await pool.query(
    `UPDATE app_incentive_special_reward
        SET is_active = $1,
            ...
            effective_to = $5::date
      WHERE reward_code = $6`,
    [isActive, targetAmount, rewardAmount, effectiveFrom, effectiveTo, rewardCode],
  );
  ...
  await pool.query(
    `INSERT INTO app_incentive_special_reward
     ...
     VALUES ($1, $2, $3, $4,
             $5, $6, $7, $8,
             $9::date, $10::date)`,
    [rewardCode, description, groupCode, brandCode, targetAmount, rewardAmount, splitByShare, isActive, effectiveFrom, effectiveTo],
  );
  return NextResponse.json(await listRewards());
}
...
export async function DELETE(request: NextRequest) {
  ...
  const removed = await pool.query(
    `DELETE FROM app_incentive_special_reward WHERE reward_code = $1`,
    [code],
  );
