...
export async function GET(request: NextRequest) {
  ...
  const [targetsRes, rewardsRes, unitRewardsRes, pointsRes, statusesRes] = await Promise.all([
    pool.query<{ employee_code: string; fullname_lo: string | null; nickname: string | null; ce: string | number | null; ac: string | number | null }>(
    ...
    pool.query<Record<string, string | number | boolean | null>>(
      `SELECT reward_code, description, group_code, brand_code, target_amount,
        ...
        WHERE effective_from < make_date($1, $2, 1) + INTERVAL '1 month'
  ...
  const unitRewards = unitRewardsRes.rows;
  const points = pointsRes.rows;
  const statuses = statusesRes.rows;

  const wb = XLSX.utils.book_new();
  wb.Props = { Title: `Incentive ${mm}/${year}`, Subject: "Monthly retail incentive configuration", Author: user.employeeCode ?? "ODG" };
